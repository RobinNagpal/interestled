...
| A13 | No clutter | One visual per card; groups on the map collapse, so a wide subject is still one screen; chrome hidden during drills; no badges or notifications mid-session; a topic that is finished with is archived off the list in one confirmed press, rather than read past every time |
